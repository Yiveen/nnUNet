...
class ExtendUNetDecoder(UNetDecoder):
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder):
            ...
            final_layers_key.append(encoder.conv_op(input_features_skip, 4, 1, 1, 0, bias=True))#Hard-code here
...
class NOENDUNetDecoder(nn.Module):
    def __init__(self,
                 encoder: Union[PlainConvEncoder, ResidualEncoder],
                 num_classes: int,
                 n_conv_per_stage: Union[int, Tuple[int, ...], List[int]],
        ...
        n_stages_encoder = len(encoder.output_channels)  # 6
        if isinstance(n_conv_per_stage, int):
            n_conv_per_stage = [n_conv_per_stage] * (n_stages_encoder - 1)
        assert len(n_conv_per_stage) == n_stages_encoder - 1, "n_conv_per_stage must have as many entries as we have " \
                                                              "resolution stages - 1 (n_stages in encoder - 1), " \
        ...
        input_features_below = encoder.output_channels[-(n_stages_encoder-1)]
        input_features_skip = encoder.output_channels[-((n_stages_encoder-1) + 1)]
        stride_for_transpconv = encoder.strides[-(n_stages_encoder-1)]
        transpconvs.append(transpconv_op(
            input_features_below, input_features_skip, stride_for_transpconv, stride_for_transpconv,
            bias=encoder.conv_bias
        ))
        # input features to conv is 2x input_features_skip (concat input_features_skip with transpconv output)
        stages.append(StackedConvBlocks(
            n_conv_per_stage[(n_stages_encoder-1) - 1], encoder.conv_op, input_features_skip, input_features_skip,
            encoder.kernel_sizes[-((n_stages_encoder-1) + 1)], 1, encoder.conv_bias, encoder.norm_op, encoder.norm_op_kwargs,
            encoder.dropout_op, encoder.dropout_op_kwargs, encoder.nonlin, encoder.nonlin_kwargs, nonlin_first
        ))
    ...
    def forward(self, skips):
        ...
        x = self.transpconvs[-1](lres_input)
        x = self.stages[-1](x)
        seg_outputs.append(self.seg_layers[-1](x))
        # invert seg outputs so that the largest segmentation prediction is returned first
        seg_outputs = seg_outputs[::-1]
