...
class DownsampleSegForDSTransformContinuous(AbstractTransform):
    '''
    data_dict['output_key'] will be a list of segmentations scaled according to ds_scales
    '''
    '''
        ...
        for s in self.ds_scales:  # 遍历所有下采样比例
            ...
            if all([i == 1 for i in s]):
                ...
                for b in range(data_dict[self.input_key].shape[0]):  # 遍历第一维
                    for c in range(data_dict[self.input_key].shape[1]):  # 遍历第二维
                        # 对每个分割图进行下采样，并将结果存储在 out_seg 中
                        out_seg[b, c] = resize_continuous(data_dict[self.input_key][b, c], new_shape[2:])
                output.append(out_seg)  # 将下采样后的分割图添加到输出列表
        data_dict[self.output_key] = output  # 将输出列表存储在 data_dict 的输出键下
        return data_dict  # 返回更新后的 data_dict
def resize_continuous(data, new_shape, order=3):
    """
    重采样连续数据（如高斯核）到新的形状。
    :param data: 输入数据，例如高斯核。
    :param new_shape: 重采样后的新形状。
    :param order: 插值的顺序，1表示双线性插值，3表示三次插值。
    :return: 重采样后的数据。
    """
    resized_data = resize(data, new_shape, order=order, mode="edge", clip=True, anti_aliasing=True)
    return resized_data